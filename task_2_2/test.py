...
with open(os.path.join(os.path.dirname(__file__), "tests_shuffle.txt"), "r") as f:    
    for line in f.readlines():
        ...
        for i in range(0, size * shuffle_val // 100):
            a[random.randint(0, size - 1)] = random.randint(min_val, max_val)
        
        result = subprocess.run([os.path.join(os.path.dirname(__file__), "./task_2_2"), os.path.join(os.path.dirname(__file__), "plots_data_shuffle.csv"), str(shuffle_val)],
                                input=" ".join(list(map(str, a))),
                                capture_output=True,
                                text=True)
